...
def main():
    print("\n\nPara Una Mejor Experiencia, tenga la terminal en Pantalla Completa...\n")
        ...
        do {
          ...
          switch(opcion) {
              ...
              while (err2 == true) {
                  try {
                      System.out.print(
                          "Este menu solo esta disponible para administradores. Ingrese su Matricula: ");
                          ...
                          for (int cont=0; cont < listaClientes.size(); cont++) {
                              if (listaClientes.get(cont).getMat().equals(matricula)) {
                                  while (val == false & & tryP > 0) {
                                      System.out.println(
                                          "Administrador " + matricula + ", escriba su contrasenia. Tiene " + tryP + " intento(s).");
                                      password = lc.nextLine();

                                      if (listaClientes.get(cont).getPass().equals(password)) {
                                          System.out.println(
                                              "Contrasenia es correcta. Bienvenido(a) " + listaClientes.get(cont).getNom() + ".");
                                          val = true;} else {
                                          System.out.println(
                      ...
                      err2 = false;} catch(InputMismatchException ime) {
                      System.err.println(
                          "Error: No se pueden teclear letras. \n");
                      lc.nextLine();}
                  }
              if (val) {
                  menu2();} else {
  ...
  public static void retirosMenu() {
    ...
    while (err) {
      try {
        ...
        err = false; } catch(InputMismatchException ime) {
        System.err.println("Error: No se pueden teclear letras. \n");
        lc.nextLine(); }
    }
    menu(); } // fin RETIROS
    ...
    while (err) {
      try {
        do {
          ...
          System.out.println(
              "Direccion del Cliente(no teclee comas por favor)");
          ...
          do {
            System.out.print(
                "Escriba los primeros 5 digitos de su numero de telefono movil, incluyendo el codigo LADA de su ciudad:");
          ...
          do {
            System.out.print(
                "Escriba los ultimos 5 digitos de su numero de telefono movil:");
            telefono2 = lc.nextInt();
            telefono2S = "" + telefono2;
            if (telefono2S.length() == 5) {
              val = true; } else {
              System.out.print(
                  "No escribio un numero de 5 digitos. Vuelva a intentar.");
              val = false; }
          } while (val == false);
          lc.nextLine();
          ...
          do {
            System.out.printf(
                "Este cliente es un cliente normal o un administrador(puede acceder al menu de administracion)? [N/A]: ");
            pre = lc.nextLine();
            if (pre.equalsIgnoreCase("N")) {
              c = new Cliente(nom, ap, direccion, dia, mes, anio, telefono);
              System.out.printf("Se ha creado al Cliente %s con el folio %d y matricula %s.%n%n", nom, c.getID(), c.getMat()); } else if (pre.equalsIgnoreCase("A")) {
              System.out.print("Escriba la contraseña del administrador: ");
              password = lc.nextLine();
              c = new Cliente(nom, ap, direccion, dia, mes, anio, telefono, password);
              System.out.printf("Se ha creado al Cliente %s con el folio %d y matricula %s.%n%n", nom, c.getID(), c.getMat()); } else {
          ...
          do {
            ...
            if (ansCard.equalsIgnoreCase("Y"))
              ...
              System.out.println("Respuesta no válida. Seleccione 'N' o 'Y'."); } while (!ansCard.equalsIgnoreCase("y") & & !ansCard.equalsIgnoreCase("n"));

          System.out.printf("Quieres agregar un cliente nuevo? [Y/N]: ");
          ans = lc.nextLine(); } while (ans.equalsIgnoreCase("Y"));

        err = false; } // fin try
      catch(InputMismatchException ime) {
        System.err.println(
            "Error: No se pueden teclear letras ni en la fecha de nacimiento ni el numero telefonico. \n");
        lc.nextLine(); } catch(ArithmeticException ae) {
        System.err.println("ArithmeticException: " + ae.getMessage());
        c = new Cliente(prev);
        lc.nextLine(); }
    } // fin err

    menu2(); } // fin Agregar CLIENTE
        ...
        do {
          ...
          if (typeCard > 4 | | typeCard < 1) {
            ...
            switch(typeCard) {
                ...
                System.out.println(
                  "Su tarjeta Clasica ha quedado registrada.\n");
                selCard = true;
                break;
                case 2:
                ...
                System.out.println(
                    "Su tarjeta Platino ha quedado registrada\n");
                selCard = true;
                break;
                case 4:
      ...
      try {
        ...
        while (idfound) {
          ...
          if (opcion == -1) {
            ...
            idfound = false; } else if (opcion == 0) {
            for (int cont=0; cont < listaClientes.size(); cont++)
            ...
            switch(sel) {
                ...
                case 3:
                System.out.println(
                  "No se encontro el Id de ese cliente, intente de nuevo.");
                idfound = true;
                break;}
          } else {
            System.out.println("\nEl Id no existe");
  ...
  public static void mDC() {
                          ...
                          if (listaClientes.get(cont).getID() == opcion) {
                              ...
                              while (sel) {
                                  ...
                                  switch(change) {
                                      ...
                                      case 'D':
                                      System.out.println(
                                          "Escriba la nueva direccion del cliente (no teclee comas por favor): ");
                                      System.out.print("Calle: ");
                                      calle = lc.nextLine();
                                      System.out.print("Numero(sin el #): ");
                                      numero = lc.nextLine();
                                      ...
                                      while (val == false) {
                                          System.out.print(
                                              "Escriba los primeros 5 digitos de su numero de telefono movil, incluyendo clave de su ciudad (55, por ejemplo):");
                                          newTel2 = lc.nextInt();
                                          newTel2S = "" + newTel2;
                                      ...
                                      listaClientes.get(
                                          position).setTel(newTel);
                                      sel = false;
                                      break;
                                      default:
                                      System.out.print(
                                          "Opcion no existe, vuelva a intentar: ");
                                      sel = true;
                                      break; } // fin switch
    ...
    menu2(); } // fin modificar datos cliente
        ...
        try {
          do {
            ...
            if (typeCard > 4 | | typeCard < 1) {
              ...
              numero = Long.toString(numeroLong);
              System.out.println(
                  "El numero de tarjeta asignado es: " + numero);
              ...
              switch(typeCard) {
                  ...
                  tc = new TCredito(numero, nip, month, year, TipoTC.BLACK);
                  System.out.println(
                      "Su tarjeta BLACK ha quedado registrada\n");
                  selCard = true;
                  break;
                  default:
                  System.out.println("Opcion no valida. Elija entre 1 y 4.\n");
    ...
    try {
      ...
      for (int cont=0; cont < listaClientes.size(); cont++) {
        pw.print("1/1/" + listaClientes.get(cont).getNom() + "/" + listaClientes.get(cont).getAp() + "/" + listaClientes.get(cont).getDir() + "/" + listaClientes.get(cont).getDias() + "/"
                 + listaClientes.get(cont).getMeses() + "/" + listaClientes.get(cont).getAnios(
                 ) + "/" + listaClientes.get(cont).getTel() + "/" + listaClientes.get(cont).getMat() + "/"
      ...
      System.out.println("Clientes guardados"); } catch(Exception E) {
